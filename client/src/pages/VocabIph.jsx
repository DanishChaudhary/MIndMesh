...
export default function VocabIph({ user }) {
    ...
    useEffect(() => {
        const fetchVocab = async () => {
            try {
                setLoading(true)
                const response = await axios.get(`/api/vocab/iph/${letter}`, {
                ...
                setError('Failed to load idioms and phrases')
        ...
        return (
            <div className="max-w-4xl mx-auto p-6">
                <div className="text-center py-8">
                    ...
                    <p className="mt-4 text-gray-600">Loading idioms and phrases...</p>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="bg-gray-50 min-h-screen p-6 font-sans">
            <div className="max-w-4xl mx-auto">
                <Link to="/iph" className="text-sm text-blue-600 mb-5 hover:underline inline-flex items-center gap-2">
                ...
                <div className="flex items-center justify-between mt-4 mb-4">
                    <h1 className="text-2xl font-bold">Idioms of {letter}</h1>
                ...
                <div className="grid grid-cols-1 gap-4 mb-6">
                    {items.length === 0 ? (
                        <div className="p-4 bg-white rounded shadow text-gray-500">
                            No idioms found for this letter.
                        </div>
                    ) : (
                        items.map((item, idx) => (
                            <div key={idx} className="p-6 bg-white rounded shadow hover:shadow-md transition-all">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="pl-4 hidden sm:block">
                                        <Link
                                            to={`/quiz?letter=${letter}&type=iph&page=${page}&pageSize=${pageSize}#q`}
                ...
                <div className="flex items-center gap-2 justify-between">
                    <div className="flex gap-2 items-center">
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                            ...
                            return p === page ? (
                                <span key={p} className="px-3 py-1 bg-yellow-600 text-white rounded">
                                    {p}
                                </span>
                            ) : (
                                <button
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                    <Link
                        to={`/quiz?letter=${letter}&type=iph`}
                        className="btn-accent px-4 py-3 rounded w-full sm:w-auto text-center bg-gradient-to-r from-amber-700 to-amber-500 text-white hover:from-amber-800 hover:to-amber-600 transition-all"
                    >
                        Quiz this letter
                    </Link>
                </div>
